...
class Gaussian:
    ...
    def identity(cls, dims: list) -> 'Gaussian':
        n = len(dims)
        return cls.from_info(dims, np.zeros((n, 1)), np.diag(np.ones(n) * 0.0001))  #! Acuerda que has cambaido esto 0.00001
    ...
    def marginalize(self, dims: list):
        ...
        # Verificar la estabilidad numérica de prec_bb
        """ if prec_bb.shape[0] != 0:
            det_prec_bb = np.linalg.det(prec_bb)
            if np.abs(det_prec_bb) < 1e-10:
                print(f"prec_bb casi singular, determinante: {det_prec_bb}")
                #time.sleep(10000)
                # Agregar regularización a la diagonal:
                regularizacion = 1e-4
                prec_bb = prec_bb + np.eye(prec_bb.shape[0]) * regularizacion
                print("Prec_bb regularizada:", prec_bb) """
